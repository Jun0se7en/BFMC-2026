...
class ExtendedKalmanFilter:
    ...
    def predict(self, ax, ay, wz, angleZ, dt=None):
        if dt is None:
            dt = self.dt_imu
            
        # Chuyển đổi góc từ độ sang radian
        angleZ_rad = np.radians(angleZ)
        wz_rad = np.radians(wz)  # Angular rate in rad/s
        ...
        # Chuyển đổi gia tốc từ IMU frame sang global frame
        cos_heading = np.cos(self.state[2])  # Use current state heading
        sin_heading = np.sin(self.state[2])
        ...
        F[0, 3] = dt  # dx/dvx
        F[1, 4] = dt  # dy/dvy
        # Add heading dependence (nonlinear terms)
        F[0, 2] = -self.state[3] * sin_heading * dt - 0.5 * ax_ms2 * sin_heading * dt**2 + 0.5 * ay_ms2 * cos_heading * dt**2
        F[1, 2] = self.state[3] * cos_heading * dt + 0.5 * ax_ms2 * cos_heading * dt**2 + 0.5 * ay_ms2 * sin_heading * dt**2
        
        # Cập nhật trạng thái
        self.state[0] += self.state[3] * dt + 0.5 * ax_global * dt**2  # x
        self.state[1] += self.state[4] * dt + 0.5 * ay_global * dt**2  # y
        self.state[2] += wz_rad * dt  # Update heading with angular rate
        self.state[3] += ax_global * dt  # vx
        self.state[4] += ay_global * dt  # vy
