...
class AutoDriveDataset(Dataset):
    ...
    def __init__(self, cfg, is_train, inputsize=640, transform=None):
        ...
        self.albumentations_transform = A.Compose([
            A.OneOf([A.MotionBlur(p=0.1), A.MedianBlur(p=0.1),A.Blur(p=0.1),], p=0.2),
            A.GaussNoise(p=0.02),
            A.CLAHE(p=0.02),
            A.RandomBrightnessContrast(p=0.02),
            A.RandomGamma(p=0.02),
            A.ImageCompression(quality_lower=75, p=0.02),
            A.OneOf([A.RandomSnow(p=0.1), A.RandomRain(p=0.1), A.RandomFog(p=0.1), A.RandomSunFlare(p=0.1), A.RandomShadow(p=0.1)], p=0.2),
            A.OneOf([A.ToGray(p=0.1), A.ToSepia(p=0.1)], p=0.2),
        ],
...
#        _,seg2 = cv2.threshold(seg_label[:,:,2],1,255,cv2.THRESH_BINARY)
    ...
    def load_image(self, idx):
        data = self.db[idx]
        img, labels, seg_label, lane_label, (h0, w0), (h,w), path = \
                                        data['img'], data['labels'], data['seg_label'],\
                                        data['lane_label'], data['(h0, w0)'], data['(h,w)'], data['path']

        return img, labels, seg_label, lane_label, (h0, w0), (h,w),path