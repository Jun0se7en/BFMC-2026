...
from lib.core.tal import dist2bbox, make_anchors, bbox2dist, TaskAlignedAssigner
...
class ComputeLoss:
    # Compute losses
    def __init__(self, model, cfg, use_dfl=True):
        device = next(model.parameters()).device  # get model device

        # Define criteria
        BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([cfg.LOSS.CLS_POS_WEIGHT], device=device), reduction='none')
    ...
    def __call__(self, predictions, targets_all, shapes = None, model = None, img=None, epoch=0):
        p, p_da, p_ll = predictions
        targets, targets_da, targets_ll = targets_all
        ...
        drivable_are_pred=p_da
        _,drivable_are_gt=torch.max(targets_da, 1)

        lane_line_pred=p_ll
        _,lane_line_gt=torch.max(targets_ll, 1)
        ...
        return loss, (loss[0].item(), loss[1].item(), loss[2].item(), lseg_focal.item(), lseg_tvl.item(), 0, loss.item())