...
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, eps=1e-7):
    ...
    # Intersection area
    inter = (torch.min(b1_x2.to(b2_x2.device), b2_x2) - torch.max(b1_x1.to(b2_x1.device), b2_x1)).clamp(0) * \
            (torch.min(b1_y2.to(b2_y2.device), b2_y2) - torch.max(b1_y1.to(b2_y1.device), b2_y1)).clamp(0)

    b1_x2 = b1_x2.to(b2_x2.device)
    b1_y2 = b1_y2.to(b2_y2.device)
    b1_x1 = b1_x1.to(b2_x1.device)
    b1_y1 = b1_y1.to(b2_y1.device)
    # Union Area
    w1, h1 = b1_x2 - b1_x1, b1_y2 - b1_y1 + eps
    w2, h2 = b2_x2 - b2_x1, b2_y2 - b2_y1 + eps
    union = w1 * h1 + w2 * h2 - inter + eps

    union = union.to(inter.device)
...
def find_3_positive(p, targets, det):
    ...
    return indices, anch
def build_targets_ota(predictions, targets, model, imgs):
    # Build targets for compute_loss(), input targets(image,class,x,y,w,h)
    det = model.module.model[model.module.detector_index] if is_parallel(model) \
        else model.model[model.detector_index]  # Detect() module
    device = torch.device(targets.device)
    indices, anch = find_3_positive(predictions, targets, det)
...
def build_targets(predictions, targets, model):
    ...
    '''
    # Build targets for compute_loss(), input targets(image,class,x,y,w,h)
    det = model.detect if is_parallel(model) \
    ...
    for i in range(det.nl):
        ...
        if nt:
            ...
            # Offsets
            gxy = t[:, 2:4]  # grid xy
            gxi = gain[[2, 3]].to('cuda') - gxy  # inverse
            j, k = ((gxy % 1. < g) & (gxy > 1.)).T
            l, m = ((gxi % 1. < g) & (gxi > 1.)).T
            j = torch.stack((torch.ones_like(j), j, k, l, m))
            t = t.repeat((5, 1, 1))[j]
            offsets = (torch.zeros_like(gxy)[None] + off[:, None].to('cuda'))[j]
        else:
            t = targets[0]
            offsets = 0
