...
class TotalLoss(nn.Module):
    ...
    def __init__(self, hyp, is320=False):
        ...
        self.is320 = is320
        # class loss criteria
        self.BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.Tensor([1.0])).to('cuda')
        # object loss criteria
        self.BCEobj = nn.BCEWithLogitsLoss(pos_weight=torch.Tensor([1.0])).to('cuda')
    ...
    def forward(self, outputs, targets, model):
        ...
        seg_da, det = targets
        out_da, out_det = outputs
        
        seg_da = seg_da.to('cuda')
        det = det.to('cuda')
        out_da = out_da.to('cuda')
        # out_det = out_det.to('cuda')
        
        device = det.device
        lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        tcls, tbox, indices, anchors = build_targets(out_det, det, model)  # targets

        # Class label smoothing https://arxiv.org/pdf/1902.04103.pdf eqn 3
        cp, cn = smooth_BCE(eps=0.0)

        BCEcls, BCEobj = self.BCEcls, self.BCEobj

        # Calculate Losses
        nt = 0  # number of targets
        no = len(out_det)  # number of outputs
        balance = [4.0, 1.0, 0.4] if no == 3 else [4.0, 1.0, 0.4, 0.1]  # P3-5 or P3-6

        # calculate detection loss
        for i, pi in enumerate(out_det):  # layer index, layer predictions
        ...
        lseg = focal_da_loss * 0.3 + tversky_loss * 0.3
        loss = lbox + lobj + lcls + lseg
        return lbox.item(), lobj.item(), lcls.item(), focal_loss.item(), tversky_loss.item(), lseg.item(), loss
